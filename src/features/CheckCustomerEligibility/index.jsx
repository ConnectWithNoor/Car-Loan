  ...
  return (
    <main className='flex justify-between items-center w-10/12 p-7 font-nunito h-full'>
      <section className='flex flex-col justify-center w-7/12 h-3/5 '>
        <div>
          ...
          <p className='text-xl font-medium leading-10 w-11/12 tracking-wide'>
            ...
            dolores maiores placeat sint id porro est et ut distinctio fuga
            culpa cum consequuntur facere? Quaerat, quibusdam nobis. Lorem,
        ...
        <div className='mt-20'>
          <MarketingTag
            ...
            title='Lorem Ipsum'
            description='dolor sit amet consectetur adipisicing elit. velit dolores maio'
          />
          <MarketingTag
            Icon={AiFillCar}
            title='Lorem Ipsum'
      ...
      <section className='bg-primary p-5 w-2/5 rounded-md shadow-xl'>
        <h3 className='border-b-2 border-gray-600 text-gray-700 pb-2 font-bold text-3xl'>
          Loan Form
        </h3>
  ...
  return (
    <form className='pt-4' onSubmit={handleSubmit(onSubmit)}>
      ...
      <InputField
        ...
        id='autoMake'
        inputRef={{
          ...register('autoMake', { required: true, pattern: /^[A-Za-z]+$/ }),
        }}
        error={`${
          errors.autoMake?.type === 'required'
            ? 'Required'
            : errors.autoMake?.type === 'pattern'
      ...
      <InputField
        ...
        id='autoModel'
        inputRef={{
          ...register('autoModel', { required: true, pattern: /^[0-9]+$/ }),
        }}
        error={`${
          errors.autoModel?.type === 'required'
            ? 'Required'
            : errors.autoModel?.type === 'pattern'
            ? 'Please type numbers only'
            : ''
        }`}
      />
      ...
      <InputField
        ...
        error={`${
          errors.yearlyIncome?.type === 'required'
            ? 'Required'
            : errors.yearlyIncome?.type === 'pattern'
            ? 'Please type numbers only'
            : ''
        }`}
      />
      ...
      <InputField
        ...
        error={`${
          (errors.creditScore?.type === 'required' ? 'Required' : '') ||
          (errors.creditScore?.type === 'min'
            ? "Number can't be less than 300"
            : '') ||
          (errors.creditScore?.type === 'max'
            ? "Number can't be greater than 850"
            : '')
        } `}
      />
